...
int main(void){
	...
	while(op){
		system("cls");	printf("\n\n");
		printf("    Menu for Linear Table List On Sequence Structure \n");
		printf("-------------------------------------------------\n");
	    	...
	    	case 1:
	   		...
	   		case 3:
				if(!Lists.elem) printf("线性表组不存在！\n"); 
					...
					printf("请输入线性表名称：");
					scanf("%s", filename);
					if(AddList(Lists, filename) == OK) printf("线性表添加成功！\n");
					else printf("添加失败！线性表组已满！\n");
				} 
				getchar();
				system("pause");
				break;
		...
		printf("    	  11. MaxSubArray\n");
		printf("    	  12. SubArrayNum\n");
		printf("    	  13. SortList\n");
		printf("    	  14. SaveList\n");
		printf("    	  15. LoadList\n");
		printf("    	  0. Exit\n");
		...
		switch(op){
	    	...
	    	case 10:  
				...
				else if(!L.length) printf("线性表是空表！\n");
				else if(ListTraverse(L) == INFEASIBLE) printf("线性表不存在！\n");
				getchar();
				system("pause");
				break;
			case 11:
				if(MaxSubArray(L) == INFEASIBLE) printf("线性表不存在或为空！\n");				
				else printf("数组具有最大和的子数组，其和为 %d \n", MaxSubArray(L));
				getchar();
				system("pause");
				break;
			case 12:
				if(!L.elem) printf("线性表不存在！\n");
					...
					printf("请输入需要查询的子数组和：");
					scanf("%d", &k);
					if(!SubArrayNum(L, k)) printf("不存在和为 %d 的子表！\n", k);
					printf("该线性表拥有 %d 个和为 %d 的子数组\n", SubArrayNum(L, k), k);
				}
				getchar();
				system("pause");
				break;
			...
			case 15:
				if(!L.elem) printf("线性表不存在！\n");
				else if(L.length) printf("线性表不为空！请先清空表！\n");
				else{
					printf("请输入源文件名：");
					scanf("%s", filename);
